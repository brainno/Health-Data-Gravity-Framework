...
import { pick, esc, GP, signOut } from './hdgf-core.js';
...
export function topbar({ lang, engine, session, patient, careHref, agentHref, showPatients }) {
  ...
  const who = !session ? '' : isPhys
    ? `<span class="who"><span class="av">HP</span><span class="who-text">${esc(pick(GP.name, lang))}<small>${esc(pick(GP.clinic, lang))}</small></span></span>`
    : `<span class="who"><span class="av">${esc(patient ? patient.initials : '')}</span><span class="who-text">${esc(patient ? patient.name : '')}<small>${t('patient')}</small></span></span>`;
